//
//  https://gist.github.com/Ahti/8662879
//  GPUImageiOSImageEffect.m
//
...
@interface GPUImageiOSImageEffect()
...
- (id)init;
{
    ...
    [self addFilter:self.colorGenerator];
    ...
    [self.blurFilter addTarget:self.colorGenerator];
    [self.colorGenerator addTarget:self.blendFilter];
    
    self.initialFilters = @[self.saturationFilter];
...
- (void)setInputSize:(CGSize)newSize atIndex:(NSInteger)textureIndex;
{
    ...
    {
        CGSize rotatedSize = [self.saturationFilter rotatedSize:newSize forIndex:textureIndex];
        
        [self.saturationFilter forceProcessingAtSize:CGSizeMake(rotatedSize.width / _downsampling, rotatedSize.height / _downsampling)];
        [self.colorGenerator forceProcessingAtSize:rotatedSize];
        [self.blendFilter forceProcessingAtSize:rotatedSize];
    }
...
- (void)setEffectType:(GPUImageIOSImageEffectType)effectType
{
    ...
    switch (effectType) {
        case GPUImageIOSImageEffectTypeLight:
          //  [self.colorGenerator setColorRed:1 green:1 blue:1 alpha:1];
            self.blendFilter.mix = 0.3;
            break;
        case GPUImageIOSImageEffectTypeExtraLight:
            [self.colorGenerator setColorRed:0.97 green:0.97 blue:0.97 alpha:1];
            self.blendFilter.mix = 0.82;
            break;
        case GPUImageIOSImageEffectTypeDark:
          //  [self.colorGenerator setColorRed:0.11 green:0.11 blue:0.11 alpha:1];
            self.blendFilter.mix = 0.73;
            break;
    }
}
...
{
    self.blurFilter.blurRadiusInPixels = newValue;
}
...
- (CGFloat)saturation;
{
    return self.saturationFilter.saturation;
}
